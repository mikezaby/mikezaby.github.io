...
We can think of modules as small functions that have the capability to be chained together.
...
In the beginning, we want to shape our project and figure out how to create, update, or remove a module.
...
`Engine.ts` exposes all the functionality that is available for the engine users.
...
As shown above, we start by defining the ModuleType enum, and then map the type to actual module props.
...
Next, we will start implementing the abstract Module class.
...
The `ICreateParams` interface serves as a helper so that we don’t need to redefine similar interfaces for the constructor parameters of the modules repeatedly.
...
We use `createModule` that be taken from `modules/index`
...
In the [next post](/posts/web-audio-engine-part3) we will start integrating WebAudio starting from AudioNode, and we will explore how we could connect modules between them.
